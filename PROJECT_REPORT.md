...
项目同时提供 FastAPI 后端、单页前端、SSE 流式输出、本地 Chroma 向量库、E0/E1/E2/E3 四组消融配置，以及 governed retrieval、agentic showcase、gradient eval 等评测脚本。它适合作为课程报告、答辩演示和 Agentic RAG 原型验证项目。
...
答辩时建议先用 E3 展示完整时间线，再切换 E0 对比工具类问题。
...
该评测更适合答辩展示“能力阶梯”：E1 体现行动正确性，E2 体现证据打分，E3 体现反思检查。
...
前端能实时展示 Agent 时间线；评测脚本能记录 route、steps、tool、top knowledge、latency 等信号，方便答辩展示和实验分析。
...
1. 说明朴素 RAG 在客服场景中的问题。
2. 引出 Agentic RAG：先判断，再行动，再生成，再自检。
3. 展示系统架构和 LangGraph 状态图。
4. 展示 governed knowledge assets 和 doc_type-aware 检索。
5. 现场演示工具调用、缺少编号、安全边界、转人工。
6. 切换 E0 和 E3 对比行为差异。
7. 展示 governed retrieval 和 gradient eval 结果。
8. 总结亮点、局限和未来工作。

面向非专业评委可以这样解释：

> 这个系统不是简单地在文档里搜索答案。它会先判断用户到底是在问政策、查订单、投诉，还是需要人工；如果是实时状态，就调用对应工具；如果是高风险承诺或隐私问题，就先看安全规则；如果资料不够，还会改写问题重新检索。右侧时间线会把每一步展示出来，所以我们能看到它为什么这样回答。

面向专业评委可以这样解释：

> 本项目使用 LangGraph 将 Agentic RAG 拆成 router、retrieve、grade、rewrite、tool、generate、reflect、handoff 等节点。知识库采用 governed assets 设计，chunk metadata 保留 doc_type、category、business_stage、visibility、priority 等字段。检索阶段先 over-fetch，再补充安全、SOP、表格、工具说明等候选，并通过 doc_type-aware rerank 排序。实验部分通过 E0/E1/E2/E3 四组消融、governed retrieval 和 gradient eval 展示各组件贡献。
...
总体而言，本项目适合作为课程项目、答辩演示和客服智能化原型，也为后续接入真实业务接口、增强检索模型、扩展多轮对话和完善安全合规提供了清晰基础。
